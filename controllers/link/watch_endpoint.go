...
func (r *endpointEventHandler) add(ctx context.Context, obj runtime.Object, queue adder) {
	...
	if cr.Status.ClaimRef != nil &&
		cr.Status.ClaimRef.APIVersion == invv1alpha1.GroupVersion.String() &&
		cr.Status.ClaimRef.Kind == invv1alpha1.LinkKind {
		r.l.Info("event requeue link", "name", cr.Status.ClaimRef.Name)
		...
		links := &invv1alpha1.LinkList{}
		if err := r.client.List(ctx, links, opts...); err != nil {
			r.l.Error(err, "cannot list links")
			return
		}
		for _, link := range links.Items {
			// links of kinf logicalinterconnect are handled seperately
			...
			if link.GetCondition(resourcev1alpha1.ConditionTypeReady).Status == metav1.ConditionFalse {
				r.l.Info("event requeue link", "name", link.GetName())
				queue.Add(reconcile.Request{NamespacedName: types.NamespacedName{
