...
func (r *reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if hasSpecificTypeConditions(cr.Status.Conditions, ct) {
		...
		_, err = fnr.Run(rl)
		if err != nil {
			r.l.Error(err, "cannot run function")
			return ctrl.Result{}, errors.Wrap(err, "cannot run function")
		}
		for _, res := range rl.Items {
			r.l.Info("resourceList", "data", res.String())
		}
	}
	return ctrl.Result{}, nil
}
...
func (r *reconciler) includeFile(path string, match []string) bool {
	for _, m := range match {
		...
		r.l.Info("includeFile", "file", file, "m", m)
		if matched, err := filepath.Match(m, file); err == nil && matched {
			r.l.Info("includeFile matched", "file", file, "m", m)
			return true
		}
	}
	return false
...
func (r *reconciler) getResourceList(resources map[string]string) (*fn.ResourceList, error) {
	...
	for path, data := range resources {
		if r.includeFile(path, []string{"*.yaml", "*.yml", "Kptfile"}) {
			r.l.Info("getResourceList resource", "path", path, "data", data)
	...
	for _, n := range pb.Nodes {
		r.l.Info("getResourceList node", "apiversion", n.GetApiVersion(), "kind", n.GetKind())
		s, err := n.String()
		if err != nil {
			return nil, err
		}
