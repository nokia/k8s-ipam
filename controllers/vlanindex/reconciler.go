...
func init() {
	controllers.Register("vlanindex", &reconciler{})
}
...
//+kubebuilder:rbac:groups=vlan.resource.nephio.org,resources=vlanindices,verbs=get;list;watch;create;update;patch;delete
//+kubebuilder:rbac:groups=vlan.resource.nephio.org,resources=vlanindices/status,verbs=get;update;patch
//+kubebuilder:rbac:groups=vlan.resource.nephio.org,resources=vlanindices/finalizers,verbs=update
...
func (r *reconciler) Setup(ctx context.Context, mgr ctrl.Manager, cfg *ctrlconfig.ControllerConfig) (map[schema.GroupVersionKind]chan event.GenericEvent, error) {
	...
	return map[schema.GroupVersionKind]chan event.GenericEvent{vlanv1alpha1.VLANIndexGroupVersionKind: ge},
		ctrl.NewControllerManagedBy(mgr).
			Named("VLANIndexController").
			For(&vlanv1alpha1.VLANIndex{}).
...
func (r *reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	// the prefixes that are within the spec of the network-instance need to be claimed first
	// since they serve as an aggregate
	if err := r.ClientProxy.CreateIndex(ctx, cr); err != nil {
		r.l.Error(err, "cannot initialize index")
		cr.SetConditions(resourcev1alpha1.ReconcileError(err), resourcev1alpha1.Failed(err.Error()))
		return ctrl.Result{Requeue: true}, errors.Wrap(r.Status().Update(ctx, cr), errUpdateStatus)
	}
